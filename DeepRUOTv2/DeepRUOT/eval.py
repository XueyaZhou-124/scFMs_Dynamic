...
from DeepRUOT.utils import euler_sdeint, euler_sdeint_reverse
import random
import ot
...
def generate_trajectories_sde_hold_start_out(df, f_net, sf2m_score_model, device, exp_dir, all_times, sigma, use_mass, num_points=None, num_runs = 1):
    n_times = np.array(all_times).max() + 1
    data=torch.tensor(df[df['samples']==all_times[1]].values,dtype=torch.float32).requires_grad_()
    ...
    for run_idx in tqdm(range(num_runs)):
        ...
        for i in range(0, len(all_times)):  # time index
            ...
            b = sde_weight_tensor[i].numpy().reshape(-1)
            b = b / b.sum()
            pred_mass = sde_weight_tensor[i].numpy().sum()
            w1 = evaluate_model(gt_data, model_i_data, a, b)
            tmv = np.abs(pred_mass - gt_mass)/gt_mass
            results.append({
                'Time Point': time_point,
...
def compute_trajectories_action(df, f_net, sf2m_score_model, device, all_times, sigma, use_mass, delta, num_points=None, num_runs = 1):
    n_times = np.array(all_times).max() + 1
    data=torch.tensor(df[df['samples']==0].values,dtype=torch.float32).requires_grad_()
    data_t0 = data[:, 1:].to(device).requires_grad_()
    x0=data_t0.to(device)
    ...
    for run_idx in tqdm(range(num_runs)):
        ...
        ts = torch.linspace(0, n_times - 1, 100, device=device)
        sde_traj, traj_lnw = euler_sdeint(sde, initial_state, dt=0.01, ts=ts)
        # sde_traj, traj_lnw = sde_traj.cpu(), traj_lnw.cpu()
        ...
        for time_idx in range(len(ts)-1):
            ...
            if sigma != 0:
                this_vel = f_net.v_net(this_t, this_x) + sf2m_score_model.compute_gradient(this_t.expand(this_x.shape[0], 1), this_x)
            else:
            ...
            loss_g = 0.5 * delta * torch.sum((this_g ** 2) * this_m)
            
            action += (loss_vel + loss_g) * dt[time_idx]

        action = action.mean().cpu().detach().item()/x0_subset.shape[0]
        results.append({
                'Model': f'DeepRUOT_run{run_idx+1}',
                'action': action,
            })
