...
def train_un1_reduce(
    ...
    for i in pbar:
        ...
        if local_loss and not global_loss:
            i_mass=1
            if hold_one_out and hold_out == 0:
                data_t0 = sample(df, groups[1], size=sample_size, replace=sample_with_replacement, to_torch=True, device = device)
            else:
                data_t0 = sample(df, groups[0], size=sample_size, replace=sample_with_replacement, to_torch=True, device = device)
            ...
            batch_loss = []
            if hold_one_out:
                groups_ho = [g for g in groups if g != hold_out] # TODO: Currently does not work if hold_out='random'. Do to_ignore before. 
                steps = generate_steps(groups_ho)
            for step_idx, (t0, t1) in enumerate(steps):  
...
def train_all(
    ...
    for batch in pbar:
        ...
        if local_loss and not global_loss:
            ...
            if hold_one_out and hold_out == 0:
                data_0 = sample(df, groups[1], size=sample_size, replace=sample_with_replacement, to_torch=True, device = device)
            else:
                data_0 = sample(df, groups[0], size=sample_size, replace=sample_with_replacement, to_torch=True, device = device)
            P0=0
            num=data_0.shape[0]
            ...
            for step_idx, (t0, t1) in enumerate(steps):  
                ...
                # Solve ODE
                x_t, lnw_t,m_t=odeint2(ODEFunc3(model,sf2m_score_model,sigmaa, use_mass, alpha),initial_state_energy,t,options=dict(step_size=0.1),atol=1e-5, rtol=1e-5, method='euler')
                lnw_t_last = lnw_t[-1]
                m_t_last=m_t[-1]
                ...
                if use_pinn:
                    ...
                    if hold_one_out and hold_out == 0:
                        data_0 = sample(df, groups[1], size=sample_size, replace=sample_with_replacement, to_torch=True, device = device)
                        time=torch.tensor([groups[1]]).to(device)
                    else:
                        data_0 = sample(df, groups[0], size=sample_size, replace=sample_with_replacement, to_torch=True, device = device)
                        time=torch.tensor([groups[0]]).to(device)
                    P0=0
                    num=data_0.shape[0]
            ...
            if reverse:
                ...
                for step_idx, (t0, t1) in enumerate(steps_reverse):  
                    ...
                    # Solve reverse ODE
                    x_t_reverse, lnw_t_reverse, m_t_reverse=odeint2(ODEFunc3(model,sf2m_score_model,sigmaa, use_mass, alpha),initial_state_energy, t, atol=1e-5, rtol=1e-5, method='euler', options=dict(step_size=0.1))
                    lnw_t_last_reverse = lnw_t_reverse[-1]
                    m_t_last_reverse = m_t_reverse[-1]
