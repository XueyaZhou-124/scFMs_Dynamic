...
def generate_state_trajectory_hold_start_out(X, n_times, batch_size,f_net,time, device):
    lnw0 = torch.log(torch.ones(batch_size,1) / (batch_size)).float().to(device)
    x1 = torch.from_numpy(X[1]).float().to(device)
    trajectory = []
    t_mid = torch.Tensor([time[1], time[0]]).float().to(device)
    m0 = torch.zeros_like(lnw0).to(device)
    initial_state_energy = (x1, lnw0, m0)
    xtt, _, _=odeint(ODEFunc2(f_net),initial_state_energy,t_mid,options=dict(step_size=0.1),method='euler')
    trajectory.append(xtt[-1].detach())
    trajectory.append(x1)
    for t_start in range(n_times - 2):
        t_mid = torch.Tensor([time[t_start + 1], time[t_start+2]]).float().to(device)
        m0 = torch.zeros_like(lnw0).to(device)
        initial_state_energy = (trajectory[-1], lnw0, m0)
        xtt, _, _=odeint(ODEFunc2(f_net),initial_state_energy,t_mid,options=dict(step_size=0.1),method='euler')
        trajectory.append(xtt[-1].detach())
...
def euler_sdeint_reverse(sde, initial_state, dt, ts):
    device = initial_state[0].device
    # Initial time, based on first timepoint in ts
    t0 = ts[0].item()
    tf = ts[-1].item()
