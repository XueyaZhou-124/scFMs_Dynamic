...
from DeepRUOT.utils import (
    ...
    generate_state_trajectory, generate_state_trajectory_hold_start_out, get_batch, get_batch_size, get_base_model
)
...
from DeepRUOT.eval import generate_trajectories_sde, generate_trajectories_sde_hold_start_out
...
class TrainingPipeline:
    ...
    def pretrain(self):
        ...
        l_loss, b_loss, g_loss = train_un1_reduce(
            self.f_net, self.df, self.groups, optimizer, self.config['pretrain']['epochs']//2,
            criterion=criterion,
            local_loss=True,
            global_loss=False,
            apply_losses_in_time=self.config['apply_losses_in_time'],
    ...
    def train_score_model(self):
        ...
        X = [adata.obsm["X_pca"][adata.obs["samples"] == self.groups[t]] for t in range(n_times)]
        ...
        if self.config['data']['hold_one_out'] and self.config['data']['hold_out'] == 0:
            trajectory = generate_state_trajectory_hold_start_out(X, n_times, self.df[self.df['samples']==self.groups[1]].x1.shape[0], self.f_net, time, self.device)
        else:
    ...
    def evaluate(self):
        ...
        if sigma == 0:
            ...
            num_runs = 1
        if self.config['data']['hold_one_out'] and self.config['data']['hold_out'] == 0:
            results = generate_trajectories_sde_hold_start_out(self.df, self.f_net, self.sf2m_score_model, self.device, self.exp_dir, self.groups, sigma, use_mass=self.config['use_mass'], num_points=None, num_runs=num_runs)
        else:
        ...
        generated = np.load(os.path.join(self.exp_dir, 'sde_point_0.npy'), allow_pickle=True)
        trajectories = np.load(os.path.join(self.exp_dir, 'sde_trajec_0.npy'), allow_pickle=True)
        plot_comparision(
            self.df, generated, trajectories,
            palette = 'viridis', df_time_key='samples',
            save=True, path=self.exp_dir, file='comparision.png',
